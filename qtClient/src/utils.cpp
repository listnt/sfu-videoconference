...
void write_ivf_file_header(std::stringbuf &ofs, const char codec[4],
...
// Write per-frame header (12 bytes): size (4), 64-bit timestamp (we'll use 4
// bytes low + 4 bytes high)
void write_ivf_frame_header(std::stringbuf &ofs, uint32_t frame_size,
                            uint64_t timestamp) {
  write_u32_le(ofs, frame_size);
  // IVF uses a 64-bit timestamp; write low dword then high dword
  // (little-endian)
...
void setupProcessMonitor(QProcess *ffplay) {
  QObject::connect(ffplay, &QProcess::stateChanged,
                   [](QProcess::ProcessState newState) {
                     qDebug() << "Process State Changed to:" << newState;
                   });

  // 2. Catch Critical Errors (e.g., File Not Found, Crashes)
  QObject::connect(ffplay, &QProcess::errorOccurred,
                   [ffplay](QProcess::ProcessError error) {
  ...
  // 4. Handle Exit
  QObject::connect(
      ffplay, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
