...
void videoPlayer::setupDecoder(std::string mid, std::string codec, bool isVideo)
{
...
void videoPlayer::setupVisuals(std::string mid, bool isVideo)
{
...
void videoPlayer::play(rtc::binary frame, std::string mid, std::string codec, bool isVideo)
{
    ...
    if (!this->mp[mid] && !this->audioSink[mid]) {
        std::call_once(*this->processed[mid], [this, mid, codec, isVideo]() {
            this->setupDecoder(mid, codec, isVideo);
            this->setupVisuals(mid, isVideo);
        });
    }
